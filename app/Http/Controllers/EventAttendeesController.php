...
class EventAttendeesController extends MyBaseController
{
    ...
    /**
     * Show the offline create tickets dialog
     *
     * @param $event_id
     * @return View
     */
     public function showOfflineTickets(Request $request, $event_id)
     {
        $event = Event::scope()->find($event_id);
   ...
   /**
    * Create offfline tickets
    *
    * @param Request $request
    * @param $event_id
    * @return mixed
    */
   public function postOfflineTickets(Request $request, $event_id)
   {
     ...
     $attendee_last_name = trans("Attendee.offline_ticket_lastname");
     $attendee_email = $event->organiser->email;
     $email_attendee = '0';
     ...
     try {
         ...
         $order->email = $attendee_email;
         $order->order_status_id = config('attendize.order.complete');
         $order->amount = $ticket_price * $ticket_qty;
         $order->account_id = Auth::user()->account_id;
         $order->event_id = $event_id;
         $order->order_reference = Utils::getOrderReference();
         ...
         /* Should offline tickets add to total tickets sold? yes
          * Update qty sold
          */
         $ticket = Ticket::scope()->find($ticket_id);
         $ticket->increment('quantity_sold', $ticket_qty);
         $ticket->increment('sales_volume', $ticket_price * $ticket_qty);
          ...
          */
         $event_stats = new EventStats();
         $event_stats->updateTicketsSoldCount($event_id, $ticket_qty);
         $event_stats->updateTicketRevenue($ticket_id, $ticket_price * $ticket_qty);

         /*
          * Create the attendees
          * Using dummy attendee info as there are offline tickets
          */
         ...
         session()->flash('message', trans("Controllers.offline_tickets_successfully_created"));
